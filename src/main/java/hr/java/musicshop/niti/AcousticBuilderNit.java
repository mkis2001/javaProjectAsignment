...
public class AcousticBuilderNit implements Runnable, LoadFile {
    int price;
    int brojac;
    AkusticnaGitara gitara;
    String user;
    public AcousticBuilderNit(int price, int brojac, AkusticnaGitara gitara) {
        this.price = price;
        this.brojac = brojac;
        this.gitara = gitara;
        user = getCurrentUser();
    ...
    public void run() {
        ...
        Platform.runLater(
                () -> {
                    try {
                        BazaPodataka.addCustomAcoustic(gitara);
